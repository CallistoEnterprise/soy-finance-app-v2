...
export default function AddLiquidity() {
  ...
  const {tokenBalance: balanceFirst} = useTokenBalance({address: tokenA?.address, chainId});
  const {tokenBalance: balanceSecond} = useTokenBalance({address: tokenB?.address, chainId});
  ...
  return <div>
    ...
    <div className={styles.summary}>
      <div className={styles.infoRow}>
        ...
        {tokenA && tokenB && priceA && priceB ? <span className={styles.priceContent}>
          ...
          <button onClick={() => setReversed(!reversed)} className={styles.showInvertedButton}>
        ...
        </span> : <span className={styles.priceContent}>
            —
        </span>}
      </div>
      {/*<div className={styles.infoRow}>*/}
...
function Action() {
  ...
  const {tokenBalance: balanceA} = useTokenBalance({address: tokenA?.address, chainId})
  const {tokenBalance: balanceB} = useTokenBalance({address: tokenB?.address, chainId})
  ...
  if (approveStatusA[0] !== ApprovalState.APPROVED || wasApprovingTokenA &&
    ...
    return <div className={styles.stepRows}>
      <div className={styles.stepRow}>
        ...
        <div>
          {approveStatusA[0] === ApprovalState.NOT_APPROVED &&
          <Button fullWidth onClick={approveStatusA[1]}>Approve {tokenA.symbol}</Button>}
          {approveStatusA[0] === ApprovalState.PENDING &&
          <Button fullWidth disabled>Approving {tokenA.symbol}...</Button>}
          {approveStatusA[0] === ApprovalState.APPROVED && wasApprovingTokenA &&
          <Button fullWidth disabled>Approve {tokenA.symbol}</Button>}
        </div>
      </div>
